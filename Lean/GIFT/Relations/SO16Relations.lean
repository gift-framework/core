...
/-- Simplified: 248 = 8 × 31 -/
theorem dim_E8_factored : Algebra.dim_E8 = 8 * 31 := by
  unfold Algebra.dim_E8
  native_decide

/-- Relation 68: 31 = 2^Weyl - 1 (Mersenne prime connection) -/
theorem mersenne_from_weyl : 2^Algebra.Weyl_factor - 1 = 31 := by
  unfold Algebra.Weyl_factor
  native_decide
...
theorem weyl_E8_factorization :
    weyl_E8_order = 2^G2.dim_G2 * 3^Algebra.Weyl_factor * Algebra.Weyl_factor^2 * 7 := by
  unfold weyl_E8_order G2.dim_G2 Algebra.Weyl_factor
  native_decide
...
theorem weyl_exponents :
    (14 = G2.dim_G2) ∧ (5 = Algebra.Weyl_factor) ∧ (7 = 7) := by
  unfold G2.dim_G2 Algebra.Weyl_factor
...
theorem geometric_120 : b2 + b3 + G2.dim_G2 + Algebra.rank_E8 = 120 := by
  native_decide

/-- Component breakdown -/
theorem geometric_components :
    (b2 = 21) ∧ (b3 = 77) ∧ (G2.dim_G2 = 14) ∧ (Algebra.rank_E8 = 8) := by
  unfold b2 b3 G2.dim_G2 Algebra.rank_E8
...
theorem spinor_from_Im_O : (2 : ℕ)^Octonions.imaginary_count = 128 := by
  unfold Octonions.imaginary_count
  native_decide
...
theorem all_SO16_relations :
    ...
    (b2 + b3 + G2.dim_G2 + Algebra.rank_E8 = 120) ∧
    (b2 = dim_SO 7) ∧
    ((2 : ℕ)^7 = 128) ∧
